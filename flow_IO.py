...
# first 4 bytes in flo file; check for this when READING the file
...
def readFloFlow(filepath):
    ...
    with open(filepath, "rb") as stream:
        ...
        if height < 1 or height > 99999:
            raise IOError(
                f"read flo file({filepath}): illegal height {height}")
...
def writeFloFlow(flow, filepath):
    ...
    """
    if type(flow) == tuple:
        flow = np.concatenate([np.expand_dims(f, -1) for f in flow], axis=-1)
    assert flow.shape[-1] == 2, 'Flow doesnt have flow-like shape'
    height, width, nBands = flow.shape

    with open(filepath, "wb") as f:
        if f is None:
            raise IOError(
                f"write flo file {filepath}: file could not be opened")
        ...
        for i in range(height):
            ...
            if result != n * 4:
                raise IOError(
                    f"write flo file {filepath}: problem writing row {i}")
