...
def colorplot_dark(*flow, auto_scale=True, max_scale=-1, transform=None, return_max=False):
    ...
    if type(flow) == tuple and len(flow) == 2:
        flow = [img.unsqueeze(-1) for img in flow]
        flow = np.concatenate(flow, axis=-1)
    elif len(flow) == 1:
        flow = flow[0]
    ...
    hue = -np.arctan2(flow[:, :, 1], flow[:, :, 0]
                      ) % (2 * np.pi) / (2 * np.pi) * 360
    hue[hue < 90] *= 60 / 90
    hue[(hue < 180) & (hue >= 90)] = (
        hue[(hue < 180) & (hue >= 90)] - 90) * 60 / 90 + 60
...
def colorplot_light(*flow, auto_scale=True, max_scale=-1, return_max=False):
    ...
    import torch
    if type(flow) == tuple and len(flow) == 2:
        if torch.is_tensor(flow[0]):
            flow = [img.cpu().detach().numpy() for img in flow]
        flow = [np.expand_dims(img, -1) for img in flow]
        flow = np.concatenate(flow, axis=-1)
