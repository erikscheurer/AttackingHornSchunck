...
def showFlow(filepath):
    ...
    dir_entries = [os.path.join(dir_name, i)
    ...
    slider = Slider(axslider, "max", valmin=0, valmax=200,
                    valinit=max_scale, closedmin=False)
    buttons = RadioButtons(axbuttons, [
                           "Color Light", "Color Dark", "Color Log", "Color LogLog", "Error", "Error Fl"])
    ...
    def updateEverything():
        ...
        colorvis = getFlowVis(
            flow, vistype=buttons.value_selected, max_scale=slider.val, gt=gt_flow)
    ...
    def update(val):
        val = slider.val
        colorvis = getFlowVis(
            flow, vistype=buttons.value_selected, max_scale=val, gt=gt_flow)
        ax_implot.set_data(colorvis)
        fig.canvas.draw_idle()
