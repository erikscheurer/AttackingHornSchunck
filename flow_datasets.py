...
SINTEL_TRAIN_SEQUENCES = ["alley_1", "alley_2", "ambush_2", "ambush_4", "ambush_5", "ambush_6", "ambush_7", "bamboo_1", "bamboo_2", "bandage_1", "bandage_2",
                          "cave_2", "cave_4", "market_2", "market_5", "market_6", "mountain_1", "shaman_2", "shaman_3", "sleeping_1", "sleeping_2", "temple_2", "temple_3"]
SINTEL_TEST_SEQUENCES = ["ambush_1", "ambush_3", "bamboo_3", "cave_3", "market_1", "market_4",
                         "mountain_2", "PERTURBED_market_3", "PERTURBED_shaman_1", "temple_1", "tiger", "wall"]
SINTEL_TRAIN_IMG_COUNTS = [50, 50, 21, 33, 50, 20, 50, 50,
                           50, 50, 50, 50, 50, 50, 50, 40, 50, 50, 50, 50, 50, 50, 50]
...
def getTrainDataset(dataset_name, sintel_imagetype=None, kitti_flowtype="flow_occ"):
    ...
    if dataset_name not in SUPPORTED_DATASETS:
        raise ValueError(
            f"Dataset {dataset_name} currently not supported. Please choose one of: " + ", ".join(SUPPORTED_DATASETS))
    ...
    if dataset_name == "mpi_sintel":
        if sintel_imagetype is None:
            raise ValueError(
                "sintel_imagetype not given, must be final or clean!")
    ...
    base_image_path = os.path.join(
        dataset_basepath, d[dataset_name]["base"], d[dataset_name]["image_path"])
    base_flow_path = os.path.join(
        dataset_basepath, d[dataset_name]["base"], d[dataset_name]["flow_path"])
    ...
    for i, sequence in enumerate(d[dataset_name]["sequences"]):
        ...
        for frame in range(d[dataset_name]["start_frame"], end_frame + 1):
            image_path = d[dataset_name]["image_format"].format(
        ...
        for frame in range(d[dataset_name]["start_frame"], end_frame):
            flow_path = d[dataset_name]["flow_format"].format(
                seq=sequence, frame=frame)
...
def getSintelTest(sintel_imagetype):
    ...
    for i, sequence in enumerate(SINTEL_TEST_SEQUENCES):
        ...
        for frame in range(1, end):
            result[sequence]["images"].append(os.path.join(
                basepath, sequence, f"frame_{frame:04d}.png"))
    return result
...
def getKITTI15Test():
    ...
    for seq in range(200):
        seq_name = f"{seq:06d}"
        images = [os.path.join(
            basepath, f"{seq_name}_{i}.png") for i in [10, 11]]
